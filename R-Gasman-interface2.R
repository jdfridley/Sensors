...
#4. Original code for R-Arduino interface, March 2018, modified Oct 2018.
...
for(ab in 1:2) { #loop over above then belowground measurements
	...
	CO2 <- rep(NA, nObs) #CO2 input vector
	...
	while(i < nObs) {
		input = as.numeric(str_extract_all(read.serialConnection(con),"\\(?[0-9,.]+\\)?")[[1]][1])
    		#print(paste(tail(j,n=1),i,input,time.zero,sep="-"))
		#if(Sys.time() > time+nObs) break #if duration has exceeded logging interval, stop
		Sys.sleep(.5) #inserted because otherwise script runs too fast for new con value
    		if(is.na(input)) {Sys.sleep(.5); next} #weird inputs skipped
    		CO2[i] <- input
		sec[i] <- Sys.time()
		...
		CO2j = c(CO2j,CO2[i])			#ultimate CO2 vector
    		#if(i>1) plot((1:i)[-1],CO2[1:i][-1],ylim=c(min(CO2[-1],na.rm=T),max(CO2[-1],na.rm=T)),xlim=c(0,nObs),ylab="CO2 ppm",xlab="",pch=19,cex.lab=1.3,cex.axis=1.3)
    		#if(length(j)>1) plot(j[-1],CO2j[-1],ylim=c(min(CO2j[-1],na.rm=T),max(CO2j[-1],na.rm=T)),ylab="CO2 ppm",xlab="",pch=19,cex.lab=1.3,cex.axis=1.3)
		...
		if(length(j)>1) {plot(j[-1],CO2j[-1],ylim=c(550,700),ylab="CO2 ppm",xlab="",pch=19,cex.lab=1.3,cex.axis=1.3)
		mtext(paste("Plant ",plant,": ",ab.lab[ab]," flux"),side=3,cex=1.5,line=2)
		par(new = T)
		plot(j[-1],PARj[-1],axes=F,xlab=NA,ylab=NA,col="blue",pch=19,cex=1)
		axis(side = 4); mtext(side = 4, line = .2,'PAR',col="blue")
		}
	...
	#append data to file once all readings for given chamber are complete
	write.table(data.frame(sec,CO2,plant,c("A","B")[ab],par,temp,rh),outfile2,append=T,col.names=F)
